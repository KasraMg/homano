    ...
    return (
        <div className={`${className ?? ''} flex flex-col items-center gap-2`}>
            ...
            <div className={`${updateQuantityMutation.isPending ? 'opacity-35' : ""} transition-opacity border-neutral-04 flex gap-3 h-8 w-20 items-center justify-between rounded-md border px-2`}>
                <Plus className="cursor-pointer text-black/900" size={16} onClick={() => updateQuantityMutation.mutate({ id: data._id, action: 'plus' }, { onSuccess: () => setQuantity(p => p + 1) })} />
                <span className="text-xs text-black/900">{quantity}</span>
                <Minus className={`${quantity == 1 ? 'pointer-events-none opacity-30' : ""} cursor-pointer text-black/900`} size={16} onClick={() => updateQuantityMutation.mutate({ id: data._id, action: 'minus' }, { onSuccess: () => setQuantity(p => p - 1) })} />
            </div>
